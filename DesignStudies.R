...
termFreqBias1[termFreqBias1 > 5]
termFreqBias2[termFreqBias2 > 5]
termFreqControl[termFreqControl > 5]

findFreqTerms(dtmBias1, lowfreq = 5)
findFreqTerms(dtmBias2, lowfreq = 5)
findFreqTerms(dtmControl, lowfreq = 5)

#Graph the main terms and relationships
freq <- 5
cT <- 0.15
par(mfrow = c(1, 1))
plot(dtmBias1, terms = findFreqTerms(dtmBias1, lowfreq = freq + 1), corThreshold = (cT+.15), weighting = T)
plot(dtmBias2, terms = findFreqTerms(dtmBias2, lowfreq = freq), corThreshold = (cT - .05), weighting = T)
plot(dtmControl, terms = findFreqTerms(dtmControl, lowfreq = freq), corThreshold = cT, weighting = T)

#Graph the wordclouds 
